# Dockerfile otimizado para SafeEdu API
...
LABEL maintainer="SafeEdu Development Team"
LABEL version="1.0.0"
LABEL description="API SafeEdu para WorldSkills 2025 - Módulo A2"
...
RUN echo "=== Estrutura do projeto SafeEdu ===" && \
...
RUN dart compile exe bin/server.dart -o bin/safeedu_server

# Verificar se compilação foi bem-sucedida
RUN test -f bin/safeedu_server || (echo "ERROR: Compilação falhou!" && exit 1)
...
COPY --from=build /app/bin/safeedu_server /app/server
...
RUN echo "logs_directory=/app/logs" > /app/config.txt && \
    echo "api_name=SafeEdu" >> /app/config.txt && \
    echo "company=SafeEdu" >> /app/config.txt
...
LABEL org.opencontainers.image.description="API para aplicativo SafeEdu - WorldSkills 2025 Módulo A2"
LABEL org.opencontainers.image.version="1.0.0"
LABEL org.opencontainers.image.created="2025-07-14"
LABEL org.opencontainers.image.authors="WorldSkills Team - SafeEdu"
LABEL org.opencontainers.image.url="https://bibliotech-api.fly.dev"
LABEL org.opencontainers.image.documentation="README.md"
LABEL org.opencontainers.image.source="https://github.com/safeedu/safeedu-api"
...
LABEL env.API_NAME="Nome da API (SafeEdu)"
LABEL env.COMPANY="Nome da empresa (SafeEdu)"